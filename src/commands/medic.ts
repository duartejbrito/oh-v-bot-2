...
export const name = "medic";

export const data = new SlashCommandBuilder()
  .setName(name)
  .setDescription("Commands related to Medic/Trunk")
  .setContexts([InteractionContextType.Guild])
  .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
  .addSubcommand((subcommand) =>
    subcommand
      .setName("setup")
      .setDescription("Setup for Medic/Trunk alerts.")
      ...
      .addBooleanOption((option) =>
        option
          .setName("auto-delete")
          .setDescription(
            "Toggle Medic/Trunk alerts to auto delete before the next post."
          )
          .setRequired(false)
      )
  );

async function setupMedicChannel(interaction: CommandInteraction) {
  ...
  if (checkPermission) {
    const content =
      ...
      utils.discord.PermissionErrorType.NO_SENDABLE_CHANNEL
        ...
        : t(TranslationKey.medic_channel_alert_error).format(
  ...
  await MedicChannel.findOrCreate({
  ...
  await (discordChannel as SendableChannels).send({
    content: t(TranslationKey.setup_medic_channel_ping).format(
      discordChannel.toString()
    ),
  });
  ...
  const message = await interaction.editReply({
    content: t(TranslationKey.setup_medic_success).format(
      discordChannel.toString(),
...
export async function execute(interaction: CommandInteraction) {
  ...
  if (options.getSubcommand() === "setup") {
    await setupMedicChannel(interaction);
  }
}
